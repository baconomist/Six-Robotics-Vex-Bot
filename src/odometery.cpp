    ...
    void initialize() {
        ...
        profileController =
                AsyncMotionProfileControllerBuilder()
                        .withLimits(defaultPathFinderLimits)
                        .withOutput(chassisController)
                        .buildMotionProfileController();
        meccanumDrive = std::dynamic_pointer_cast<XDriveModel>(chassisController->getModel());
    }

    void moveDistance(QLength distance, PathfinderLimits limits) {
        if (distance < 0_ft) {
            profileController->moveTo({{0_ft,     0_ft, 0_deg},
                                       {distance, 0_ft, 0_deg}}, limits, true);
            profileController->waitUntilSettled();
            profileController->reset();
        } else {
            profileController->moveTo({{0_ft,     0_ft, 0_deg},
                                       {distance, 0_ft, 0_deg}}, limits, false);
            profileController->waitUntilSettled();
            profileController->reset();
        }
    }
